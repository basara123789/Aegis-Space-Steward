...
export const InfiniteCanvas = forwardRef<CanvasApi, InfiniteCanvasProps>(({
  ...
  useImperativeHandle(ref, () => ({
    ...
    panTo,
    resetView,
  }), [screenToWorld, zoomIn, zoomOut, panTo, resetView]);
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (isPanning) {
      ...
      // Smart Constraint: Keep the "Capsule" element within reasonable view
      const canvas = canvasRef.current;
      const focusEl = elements.find(el => el.id === 'capsule-layout-2d');
      ...
      if (canvas && focusEl) {
        ...
        const elCenterX = focusEl.position.x + focusEl.width / 2;
        const elCenterY = focusEl.position.y + focusEl.height / 2;

        // Calculate the "Ideal Pan" that puts element in center of screen
        // idealPanX = viewportW / 2 - elCenterX * zoom
        const idealPanX = viewportW / 2 - elCenterX * zoom;
        const idealPanY = viewportH / 2 - elCenterY * zoom;

        // Allow deviation from ideal pan
        const ALLOWED_OFFSET = 300 * zoom;

        clampedX = Math.max(idealPanX - ALLOWED_OFFSET, Math.min(idealPanX + ALLOWED_OFFSET, newX));
        clampedY = Math.max(idealPanY - ALLOWED_OFFSET, Math.min(idealPanY + ALLOWED_OFFSET, newY));
      } else {
        // Fallback if no specific element
        const LIMIT = 2000;
        clampedX = Math.max(-LIMIT, Math.min(LIMIT, newX));
        clampedY = Math.max(-LIMIT, Math.min(LIMIT, newY));
      }
  ...
  }, [isPanning, startPan, groupInteraction, onUpdateMultipleElements, zoom, pan, onMouseMove, screenToWorld, elements]);
  ...
  const IMAGE_STYLES = [
    'Default', '8-Bit', 'Botanical Art', 'Comic Book', 'Cubism', 'Cyberpunk', 'Exploded View',
    'Glitch Art', 'Isometric', 'Knolling', 'Low Poly', 'Mosaic', 'Oil Painting', 'Pixel Art',
    'Playful 3D Art', 'Pop Art', 'Photorealism', 'Surrealism', 'Vaporwave', 'Vector Art', 'Watercolor',
  ];
