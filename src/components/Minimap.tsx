...
export const Minimap: React.FC<MinimapProps> = ({ elements, viewport, onPanTo, onZoomIn, onZoomOut }) => {
  ...
  return (
    <div className="flex flex-col items-end gap-2">
      <div className="bg-slate-800/80 backdrop-blur-md rounded-lg p-2 shadow-lg border border-slate-700 mb-1">
        <span className="text-xs font-bold text-blue-400 uppercase tracking-widest">Station Navigation</span>
      </div>
      <div
        className={`bg-slate-800/80 backdrop-blur-sm rounded-lg shadow-lg border border-slate-700 transition-all duration-300 ease-in-out ${isCollapsed ? 'w-auto' : 'w-[200px]'}`}
      >
        <div className="flex items-center justify-between p-1">
          <button onClick={onZoomOut} className="p-1 rounded hover:bg-slate-700 text-gray-400">
          ...
          <span className="text-xs font-semibold text-gray-400 w-10 text-center">{Math.round(viewport.zoom * 100)}%</span>
          <button onClick={onZoomIn} className="p-1 rounded hover:bg-slate-700 text-gray-400">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4" /></svg>
          </button>
          <button onClick={() => setIsCollapsed(!isCollapsed)} className="p-1 rounded hover:bg-slate-700 text-gray-400">
